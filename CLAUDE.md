...
- **`KBOld/`** : anciennes fiches KB « officielles » (héritées du principe initial d'un corpus dédié RAG, abandonné — l'entreprise ne veut qu'une seule base de connaissances). Toujours indexées pour le RAG comme avant, juste déplacées hors de la racine de `corpus/`. Servies/éditées via `/corpus/file` (préfixe `KBOld/` supporté) et listées via `/corpus/kbold/list`.
